...
[module list](#modules) turns on, and each one only does something
...
[NixOS modules](MODULES.md#nixos-modules). The two sides are independent: set `flavor`,
...
One module per program, `gruvbox.<name>.enable`, on whenever `gruvbox.enable`
is. A module maps the palette onto the program's own options and only acts when
that program is enabled, so importing the flake never installs or changes
anything on its own. Per-program details live in [MODULES.md](MODULES.md).
